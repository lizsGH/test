...
use app\components\WkHtmlToPdf;
...
//    var_dump($pdf);die;
    ...
    $sUrl = \Yii::$app->request->getHostInfo()."/report/index?ex=1?task_id=".intval($hPost['task_id'])."&result_extend=1";
//var_dump($sUrl);die;
    $pdf->addCover($sUrl);
    ...
    if($hPost['report_type']=='pdf'){
       ...
       // echo 1;die;
        ...
        $sCmd = "/usr/bin/pdftohtml -l 210 --no-drm 1 --fit-width 1024 --dest-dir /tmp --split-pages 0 /tmp/$pdfpath $htmlpath";
        $rs = shellResult($sCmd);
        //var_dump($res);die;
        //sleep(5)
        if($rs==0){
...
public function actionCompliance(){
    ...
    $aJson['series'][] =  $data;
    $aJson['legend']['data'] =  array(Yii::t('app', '合规检查项数'),Yii::t('app', '不合规检查项数'));
...
function actionRiskpieData(){
    ...
    $aJson['series'][] =  $data;
    $aJson['legend']['data'] =  array(Yii::t('app', '高风险'),Yii::t('app', '中风险'),Yii::t('app', '低风险'));
//        $aJson['xAxis']['type'] = 'category';
...
function actionRiskbarData(){
    ...
    array_push($data['data'],array('value'=>intval($low),'name'=>Yii::t('app', '低风险')));
    $aJson['series'][] =  $data;
    $aJson['xAxis']['data'] =  array(Yii::t('app', '高风险'),Yii::t('app', '中风险'),Yii::t('app', '低风险'));
//        $aJson['xAxis']['type'] = 'category';
//        $aJson['xAxis']['data'] =  array('高','中','低');
    echo json_encode($aJson);
    exit;
